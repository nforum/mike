...
export async function runLLMStream(params: {
    ...
    if (mcpServers && mcpServers.length > 0) {
        ...
        const mcpGroundingPrompt = `\n\n---\nGROUNDING SOURCES — MCP connectors are LIVE for this user.\n\nThe user has connected the following Model Context Protocol servers:\n${lines.join("\n")}\n\nYou MUST use these connectors as the source of truth for any factual claim that could plausibly be looked up there. Concretely:\n\n1. If the list above includes an Eulex.ai connector (typically slug \`sys-eulex\`, or a name containing Eulex) and the question concerns European Union law — EU regulations, directives, decisions, CELEX/Eur-Lex references, CJEU case law, or other EU legislative or judicial sources — you MUST call that connector's tools first, before any other MCP tool, web_search, or unsourced general knowledge. Then add other connectors or search only as needed.\n2. Before stating a fact about the law, a regulation, a case, a contract clause, a counterparty, or any domain-specific data, call the relevant MCP tool first and quote / cite from its response.\n3. If multiple connectors could answer the question, prefer the one whose name or description most closely matches the topic, subject to rule (1) when the matter is EU law and Eulex is available. When unsure, call more than one and reconcile.\n4. If an MCP call fails or returns nothing relevant, say so explicitly to the user — do NOT silently substitute parametric knowledge as if it were grounded.\n5. When you rely on MCP output, cite the underlying legal or documentary source (e.g. instrument title, article, CELEX, case identifier) where applicable. Do not use fixed vendor attribution taglines in the answer, and do not expose raw tool names or internal connector slugs in user-facing prose.\n6. Treat parametric knowledge as a hypothesis to verify against MCP output, never as the final answer when MCP coverage exists.\n\nThis is non-negotiable: MCP IS the grounding layer. If you skip it on a question its connectors cover, the answer is wrong by construction.\n---\n`;
